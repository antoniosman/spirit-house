...
export function advance(s, rng = Math.random) {
  ...
  const choose = (ps, score) =>
    ps
      .map((p) => ({ p, v: score(p) + rng() * 12 }))
  ...
  const abilityFor = (p) =>
    ({
      ...
      billy: "Kitsune",
      elisa: "Γοργόνα",
      evelyn: "Νεράιδα του νερού",
    })[p.name.toLowerCase()] || null;
  const triggerAbility = () => {
    const pool = alive.filter((p) => abilityFor(p) && !s.abilityUsed[p.id]);
    if (!pool.length || rng() > 0.34) return null;
    ...
    if (
      actor.name.toLowerCase() === "tony" ||
      actor.name.toLowerCase() === "evelyn"
    ) {
      s.immune = actor.id;
      text += ` Η ασπίδα προστατεύει τον ${actor.name} για αυτή την εβδομάδα.`;
    } else if (actor.name.toLowerCase() === "rino") {
      actor.wins += 2;
      text += " Η αγριότητα του δίνει δύο νίκες δύναμης.";
    } else if (
      actor.name.toLowerCase() === "luna" ||
      actor.name.toLowerCase() === "elisa"
    ) {
      const target = alive.find((p) => p.id !== actor.id);
      if (target) {
        s.relations[key(actor.id, target.id)] = Math.min(
          10,
          (s.relations[key(actor.id, target.id)] || 0) + 4,
        );
        text += ` Η μαγεία ενώνει τον ${actor.name} με τον ${target.name}.`;
      }
    } else if (actor.name.toLowerCase() === "billy" && s.nominees.length) {
      const candidates = alive.filter(
        (p) => p.id !== s.hoh && !s.nominees.includes(p.id),
      );
      const target = candidates[0];
      if (target) {
        const changed = s.nominees[0];
        s.nominees[0] = target.id;
        text += ` Ο καθρέφτης του Kitsune αλλάζει ${name(changed)} με ${target.name}.`;
      }
    }
    return event(
      `Ability unleashed: ${ability}`,
      text,
      [actor.id, ...(s.immune === actor.id ? [actor.id] : [])],
      { ability, dialogue: [{ speaker: actor.name, text }] },
    );
  };

  const scheduledTwist = () => {
    if (alive.length <= 10 && s.order.length >= 2 && !s.returnUsed)
      return "return";
    ...
    if (s.week === 7) return "publicSave";
    if (s.week === 8) return "bloodMoon";
    if (s.week > 7 && rng() < 0.22)
      return [
        "triple",
        "eclipse",
        "noVeto",
  ...
  if (alive.length === 2) {
    ...
    const votes = jury.map((j) => ({
      ...
      target: choose(
        alive,
        (p) =>
          bond(j.id, p.id) + p.wins * 1.8 + p.strategy * 0.6 + p.social * 0.4,
      ).id,
    }));
    const tally = alive
    ...
    const win =
      tally[0].n === tally[1].n
        ? choose(alive, (p) => p.wins + p.social)
        : tally[0].p;
    s.winner = win.id;
    s.stage = "final";
    return event(
      "Ο μεγάλος τελικός",
      `${win.name} κερδίζει τη σεζόν! Όλοι όσοι αποχώρησαν ψήφισαν στον τελικό.`,
      alive.map((p) => p.id),
      { votes, tie: tally[0].n === tally[1].n },
    );
  }
  ...
  if (s.stage === "hoh") {
    ...
    if (s.twist === "return" && s.order.length >= 2) {
      ...
      s.twist = null;
      s.returnUsed = true;
      s.returnChoiceId = null;
      return event(
        "Ανατροπή: Η Πύλη ανοίγει",
        `${returned.map((p) => p.name).join(" και ")} επιστρέφουν από τη Σκιά. Ο ένας επιλέχθηκε από το κοινό και ο άλλος από την τύχη.`,
        returned.map((p) => p.id),
        { twist: "return", returned: returned.map((p) => p.id) },
      );
    }
    ...
    const twistText = s.doubleRemaining
      ...
      : s.twist === "triple"
        ? " Απόψε θα υπάρξουν τρεις υποψήφιοι."
        : s.twist === "eclipse"
          ? " Η Έκλειψη μπορεί να αλλάξει έναν υποψήφιο."
          : s.twist === "immunity"
            ? ` ${name(s.immune)} προστατεύεται από τη Φωτεινή Ασπίδα.`
            : s.twist === "sacrifice"
              ? " Πριν την ψηφοφορία, κάποιος μπορεί να κάνει την απόλυτη Θυσία."
              : s.twist === "double"
                ? " Απόψε οι πόρτες θα ανοίξουν δύο φορές."
                : s.twist === "noVeto"
                  ? " Η δύναμη του veto σφραγίζεται για αυτή την εβδομάδα."
                  : s.twist === "publicSave"
                    ? ` Το κοινό σώζει τον ${name(s.immune)} πριν από τις υποψηφιότητες.`
                    : s.twist === "bloodMoon"
                      ? " Η Ματωμένη Σελήνη θα φορτώσει έναν υποψήφιο με μία επιπλέον ψήφο."
                      : s.twist === "secretRoom"
                        ? ` Ο ${name(s.immune)} βρίσκει το Μυστικό Δωμάτιο, παίρνει ασυλία και δύναμη.`
                        : "";
    const twistName = s.doubleRemaining ? "double" : s.twist;
    const twistTitles = {
      triple: "Ανατροπή: Τριπλή απειλή",
      eclipse: "Μαγική ανατροπή: Η Έκλειψη",
      immunity: "Ανατροπή: Η Φωτεινή Ασπίδα",
      sacrifice: "Μαγική ανατροπή: Ο Κύκλος της Θυσίας",
      double: s.doubleRemaining
        ? "Ανατροπή: Η νύχτα δεν τελείωσε"
        : "Ανατροπή: Διπλή αποχώρηση",
      noVeto: "Ανατροπή: Σφραγισμένο Veto",
      publicSave: "Ανατροπή: Η ψήφος του κοινού",
      bloodMoon: "Μαγική ανατροπή: Ματωμένη Σελήνη",
      secretRoom: "Ανατροπή: Το Μυστικό Δωμάτιο",
    };
    const e = event(
      twistName
        ? twistTitles[twistName]
        : alive.length === 3
          ? "Τελικός αρχηγός"
          : "Αγώνας αρχηγού",
      `${p.name} κερδίζει την εξουσία και την ασυλία της εβδομάδας.${twistText}`,
      [p.id, ...(s.immune ? [s.immune] : [])],
      twistName ? { twist: twistName } : {},
    );
    s.stage = alive.length === 3 ? "finalcut" : "nominate";
    return e;
  }
  ...
  if (s.stage === "finalcut") {
    ...
    const out = choose(
      others,
      (p) => -bond(s.hoh, p.id) + p.strategy * 0.5 + p.wins,
    );
    out.out = true;
    s.order.push(out.id);
    const e = event(
      "Μία θέση πριν τον τελικό",
      `${name(s.hoh)} επιλέγει να αποχωρήσει ${out.name}. Οι δύο φιναλίστ αντιμετωπίζουν την κριτική επιτροπή.`,
      [out.id],
      { evicted: out.id },
    );
    s.stage = "jury";
  ...
  if (s.stage === "nominate") {
    ...
    s.nominees = candidates
      ...
      .map((p) => ({
        ...
        v:
          -bond(s.hoh, p.id) +
          (p.strategy * get(s.hoh).strategy) / 16 +
          p.wins +
          rng() * 8,
      }))
      .sort((a, b) => b.v - a.v)
    ...
    let title =
      count === 3 ? "Ανατροπή: Τριπλή απειλή" : "Τελετή υποψηφιοτήτων";
    let text = `${name(s.hoh)} θέτει σε κίνδυνο τους ${s.nominees.map(name).join(", ")}.${s.cursedNominee ? ` Η Ματωμένη Σελήνη δίνει μία επιπλέον ψήφο στον ${name(s.cursedNominee)}.` : ""}`;
    if (s.twist === "eclipse") {
      ...
      if (pool.length) {
        ...
        s.nominees = s.nominees.map((id) =>
          id === original ? replacement : id,
        );
    ...
    s.stage = s.twist === "noVeto" ? "house" : "veto";
    return e;
  }

  if (s.stage === "veto") {
    const fixed = alive.filter(
      (p) => p.id === s.hoh || s.nominees.includes(p.id),
    );
    const rest = alive
      .filter((p) => !fixed.includes(p))
      .sort(() => rng() - 0.5)
      .slice(0, Math.max(0, 6 - fixed.length));
    const winner = choose(
      [...fixed, ...rest],
      (p) => p.competition + p.strategy * 0.25,
    );
    winner.wins++;
    let saved = s.nominees.includes(winner.id)
      ? winner.id
      : s.nominees.find((id) => bond(winner.id, id) > 5);
    ...
    if (saved) {
      ...
      if (candidates.length) {
        replacement = choose(
          candidates,
          (p) => -bond(s.hoh, p.id) + p.strategy * 0.4,
        ).id;
        s.nominees = s.nominees.map((id) => (id === saved ? replacement : id));
      } else saved = null;
    }
    const e = event(
      "Power of Veto",
      `${winner.name} κερδίζει το veto. ${saved ? `${name(saved)} σώζεται και ${name(replacement)} παίρνει τη θέση του.` : "Οι υποψηφιότητες παραμένουν ίδιες."}`,
      [winner.id, ...s.nominees],
    );
    s.stage = "house";
    return e;
  }
  ...
  if (s.stage === "house") {
    ...
    const activeCouples = coupleKeys
      .map((k) => k.split("|").map(get))
      .filter((pair) => pair.every(Boolean) && pair.every((p) => !p.out));
    if (activeCouples.length && rng() < 0.18) {
      const pair = activeCouples[Math.floor(rng() * activeCouples.length)],
      ...
      if (!s.permanentCouples[k]) {
        ...
        return event(
          ...
          {
            dialogue: [
              { speaker: pair[0].name, text: "Δεν σε εμπιστεύομαι πια." },
              { speaker: pair[1].name, text: "Τότε τελειώσαμε." },
            ],
            breakup: k,
          },
        );
    ...
    if (s.twist === "sacrifice") {
      const volunteers = alive.filter(
        (p) => p.id !== s.hoh && !s.nominees.includes(p.id),
      );
      if (volunteers.length) {
        const volunteer = choose(
          volunteers,
          (p) => p.social + p.strategy * 0.25,
        );
        volunteer.out = true;
        s.order.push(volunteer.id);
        const savedNames = s.nominees.map(name).join(", ");
        const e = event(
          "Μαγική ανατροπή: Η Θυσία",
          `${volunteer.name} μπαίνει στον Κύκλο και λέει «Θέλω να φύγω εγώ». Η Θυσία γίνεται δεκτή και οι ${savedNames} σώζονται.`,
          [volunteer.id, ...s.nominees],
          { twist: "sacrifice", evicted: volunteer.id, sacrifice: true },
        );
        s.previous = s.hoh;
        s.hoh = null;
        s.nominees = [];
    ...
    if (possibleRomance.length > 1 && rng() < 0.22) {
      const a = possibleRomance[Math.floor(rng() * possibleRomance.length)];
      const b = possibleRomance.filter((p) => p.id !== a.id)[
      ...
      const e = event(
        ...
        `${a.name} και ${b.name} έρχονται πιο κοντά. Η νέα τους σχέση μπορεί να αλλάξει συμμαχίες, ψήφους και αποφάσεις.`,
        [a.id, b.id],
        {
          romance: romanceKey,
          dialogue: [
            { speaker: a.name, text: "Νιώθω ότι μπορώ να σου μιλήσω." },
            { speaker: b.name, text: "Κράτα το μυστικό μας." },
          ],
        },
      );
      s.stage = "evict";
    ...
    const b = alive.filter((p) => p !== a)[
      Math.floor(rng() * (alive.length - 1))
    ];
    const delta = rng() > 0.45 ? 2 : -2;
    const k = key(a.id, b.id);
    s.relations[k] = Math.max(-10, Math.min(10, (s.relations[k] || 0) + delta));
    const e = event(
      "Δωμάτιο των μυστικών",
      delta > 0
        ? `${a.name} και ${b.name} έρχονται πιο κοντά μετά από μία ειλικρινή εξομολόγηση.`
        : `Η εξομολόγηση του ${a.name} φέρνει ένταση με ${b.name}. Η εμπιστοσύνη τους κλονίζεται.`,
      [a.id, b.id],
      {
        dialogue: [
          {
            speaker: a.name,
            text: delta > 0 ? "Χαίρομαι που το είπαμε." : "Με πλήγωσες.",
          },
          {
            speaker: b.name,
            text:
  ...
  if (s.stage === "evict") {
    ...
    const votes = alive
      .filter((p) => p.id !== s.hoh && !s.nominees.includes(p.id))
      .map((p) => ({
        voter: p.id,
        target: choose(
          nominees,
          (n) =>
            -bond(p.id, n.id) + (n.strategy * p.strategy) / 20 - n.social * 0.3,
        ).id,
      }));
    const counts = s.nominees
      .map((id) => ({
        id,
        n:
          votes.filter((v) => v.target === id).length +
          (id === s.cursedNominee ? 1 : 0),
      }))
      .sort((a, b) => b.n - a.n);
    const tie = counts[0].n === counts[1].n;
    const out = tie
      ? choose(nominees, (p) => -bond(s.hoh, p.id) + p.strategy * 0.3).id
      : counts[0].id;
    get(out).out = true;
    s.order.push(out);
    ...
    const e = event(
      continues
        ? "Πρώτη αποχώρηση — η νύχτα συνεχίζεται"
        : "Η ώρα της αποχώρησης",
      text,
      [out],
      { votes, evicted: out, ...(continues ? { twist: "double" } : {}) },
    );
    s.previous = s.hoh;
    s.hoh = null;
    s.nominees = [];
    s.immune = null;
    s.cursedNominee = null;
    if (continues) {
      s.doubleRemaining = true;
      s.stage = "hoh";
    } else {
      s.doubleRemaining = false;
      s.twist = null;
      s.week++;
      s.stage = "hoh";
    }
